...
let
  ...
  cfg = config.services.zabbixWebCaddy;
  opt = options.services.zabbixWebCaddy;
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    services.zabbixWebCaddy.extraConfig = optionalString ((versionAtLeast config.system.stateVersion "20.09") && (versionAtLeast cfg.package.version "5.0.0")) ''
      $DB['DOUBLE_IEEE754'] = 'true';
    '';

    systemd.tmpfiles.rules = [
      "d '${stateDir}' 0750 ${user} ${group} - -"
      "d '${stateDir}/session' 0750 ${user} ${config.services.caddy.group} - -"
    ];
